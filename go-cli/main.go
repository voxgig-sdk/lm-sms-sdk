// Command LmSms-cli is a CLI + REPL for the LmSms SDK,
...
const entitiesHelp = "schedule send_message"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      LM_SMS_APIKEY (api key), LM_SMS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: LM_SMS_APIKEY carries the API key and
	// LM_SMS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LM_SMS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LM_SMS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewLmSmsSDK(opts)
