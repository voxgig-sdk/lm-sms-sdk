    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LMSMS_TEST_SCHEDULE_ENTID JSON to run live")
        client = setup["client"]
        ...
        schedule_ref01_markdef_up0_value = "Mark01-schedule_ref01_" + str(setup["now"])
        schedule_ref01_data_up0_up[schedule_ref01_markdef_up0_name] = schedule_ref01_markdef_up0_value

        schedule_ref01_resdata_up0 = helpers.to_map(schedule_ref01_ent.update(schedule_ref01_data_up0_up, None))
...
def _schedule_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("LMSMS_TEST_SCHEDULE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("LMSMS_TEST_LIVE") == "TRUE":
        ...
        client = LmSmsSDK(helpers.to_map(merged_opts))

    _live = env.get("LMSMS_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("LMSMS_TEST_EXPLAIN") == "TRUE",
        "live": _live,
