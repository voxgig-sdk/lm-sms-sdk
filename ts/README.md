# LmSms TypeScript SDK



The TypeScript SDK for the LmSms API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Schedule()` — each with a small set of operations (`list`, `load`, `create`, `update`, `remove`)
...
```ts
try {
  const schedule = await client.Schedule().load({ id: 'example_id' })
  console.log(schedule)
} catch (err) {
...
```ts
const client = new LmSmsSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `LmSmsSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `LmSmsSDK.test(testopts?, sdkopts?)` | `LmSmsSDK` | Create a test-mode client. |
...
| `client` | `client(): LmSmsSDK` | Return the parent SDK client. |
