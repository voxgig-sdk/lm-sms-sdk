...
const TEST_SEED = { "entity": { "schedule": { "test01": { "id": "test01" } }, "send_message": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+LmSmsSDK\b/.test(code) ||
        /\bLmSmsSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+LmSmsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/LmSmsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
