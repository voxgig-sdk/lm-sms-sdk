...
class ScheduleEntity
{
    ...
    {
        if ($args) {
            $this->_data = LmSmsHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = LmSmsHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws LmSmsError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = LmSmsHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws LmSmsError on failure (item-5 convention).
     */
     ...
     *
     * @param ScheduleUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed ScheduleUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Schedule|array The updated Schedule as an assoc-array at the
     *   SDK boundary; throws LmSmsError on failure (item-5 convention).
     */
