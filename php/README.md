...
The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Schedule()` — with named operations (`list`/`load`/`create`/`update`/`remove`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```php
$client = LmSmsSDK::test($testopts, $sdkopts);
```
...
The main class (`lmsms_sdk.php`) exports the SDK class
