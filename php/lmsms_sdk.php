...
class LmSmsSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = LmSmsHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = LmSmsHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!LmSmsFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, LmSmsFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return LmSmsUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = LmSmsHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = LmSmsHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = LmSmsHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new LmSmsError($op . "_allow",
                "LmSmsSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = LmSmsHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new LmSmsError("graphql_error",
                "LmSmsSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
