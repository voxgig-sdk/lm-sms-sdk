...
// LmSms SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class LmSmsMakeError
{
    public static function call(?LmSmsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new LmSmsContext([], null);
        }
        $op = $ctx->op ?? new LmSmsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new LmSmsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof LmSmsError) ? $err->msg : (string)$err;
        $msg = "LmSmsSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new LmSmsError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof LmSmsError) {
