...
//    only - a typed LmSmsContext there would make every ctx
...
final class LmSmsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new LmSmsContext($clean, null);
    ...
    public static function err_from_map($m): ?LmSmsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new LmSmsError(is_string($code) ? $code : '', $msg);
    }
