...
class FtCtrl extends LmSmsControl
{
...
class FtClient
{
    ...
    private LmSmsUtility $utility;

    public function __construct(LmSmsUtility $utility, array $options)
    {
    ...
    public function get_utility(): LmSmsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LmSmsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LmSmsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LmSmsConfig::make_feature($name);
    ...
    private static function buildUrl(LmSmsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LmSmsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LmSmsResponse($fetched);
        }
    ...
    private function fail(LmSmsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LmSmsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
