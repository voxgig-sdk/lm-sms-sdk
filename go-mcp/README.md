...
[MCP](https://modelcontextprotocol.io) server exposing the LmSms SDK as
two agent tools — `lm-sms_list` and `lm-sms_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/lm-sms-mcp)
make build
...
claude mcp add --scope user lm-sms \
  -- /absolute/path/to/lm-sms-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./lm-sms-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// lm-sms_list: first page of records
{ "entity": "schedule" }
{ "entity": "schedule", "query": { } }

// lm-sms_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/lm-sms-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user lm-sms \
     -- "$PWD"/dist/*/lm-sms-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `lm-sms_list` and `lm-sms_load` tools now appear
   in new sessions. Ask the agent to *"list schedule using lm-sms"*
   and it calls `lm-sms_list` with `{"entity":"schedule"}`.
...
| Tool | Args | Returns |
|------|------|---------|
| `lm-sms_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `lm-sms_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./lm-sms-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"lm-sms_load","arguments":{"entity":"schedule","query":{"id":1}}}}'
```
